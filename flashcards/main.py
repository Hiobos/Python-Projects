...
def new_word(state):
    ...
    try:
        if state == 'wrong' and random_pick != '':
            to_add = [data_df.at[random_pick, 'French'], data_df.at[random_pick, 'English']]
            if to_add not in wrong_words:
                wrong_words.append(to_add)
            print(wrong_words)
        elif state == 'right' and random_pick != '':
            if should_remove:
                to_remove = [data_df.at[random_pick, 'French'], data_df.at[random_pick, 'English']]
    ...
    #picking random word
    random_pick = random.randrange(0, len(data_df))
    ...
    window.after(3000, lambda: update_to_english(english_word))
